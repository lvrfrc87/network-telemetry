...
def dashboard():
    ...
    template = jinja_env.get_template('ping-probes/var/dashboard_template.j2')
    for region in dic_targets.keys():
        directory = 'grafana/json/'
        file = os.path.join(directory + region.upper() + '_PING' '.json')
        with open(file, 'w+') as dashboard_file:
            dashboard_file.write(template.render(yamlVar=dic_targets))
            dashboard_file.close()

if __name__ == '__main__':
    dic_targets = yaml.load(open('ping-probes/var/targets.yaml', 'rb'))
    dashboard()
